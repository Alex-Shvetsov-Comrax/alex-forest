...
import { AddBeePastureTableService } from './add-bee-pasture-table.service';


@Component({
  selector: 'app-add-bee-pasture-table',
  templateUrl: './add-bee-pasture-table.component.html',
  styleUrls: ['./add-bee-pasture-table.component.scss']
})
export class AddBeePastureTableComponent implements OnInit, Table<AddBeePastureTableModel>
{ 
  @Input() action: string;
  ...
  constructor(private addBeePastureTableService:AddBeePastureTableService) { }

  ngOnInit(): void {
    this.dataSource = this.addBeePastureTableService.dataSource;
  ...
  public onSelected(suppliers: Observable<AddBeePastureTableModel[]>) {
    this.addBeePastureTableService.emitSelected(suppliers);
  }
